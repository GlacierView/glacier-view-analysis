...
Both inference entry points (``glacierview infer`` and ``glacierview areas``)
share this module, so a change here affects them equally — which is the point.
The pipeline is:
...
from glacierview import rasters as raster_io
from glacierview.bands import landsat_bands
from glacierview.config import (
    COMMON_BANDS,
    INDEX_EPSILON,
    MODEL_INPUT_DIM,
)
...
def prepare_glacier_stack(glacier_dir, dem_path, filtered_file_path=None):
    ...
    """
    images = raster_io.get_rasters(glacier_dir, filtered_file_path=filtered_file_path)
    if not images:
    ...
    images = get_common_bands(images, list(COMMON_BANDS))
    ...
    dem = raster_io.get_dem(dem_path)
