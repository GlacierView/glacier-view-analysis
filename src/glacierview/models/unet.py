...
This is the single definition of the model. Import `UNet` and `conv_block`
from here; do not copy the classes elsewhere. Use
`glacierview.models.load_checkpoint` rather than calling `torch.load`
directly — it handles the pickle quirk described below.
...
consequences:
  ...
  * Checkpoints written before this package existed reference `__main__.UNet`
    and `__main__.conv_block`, so both names must be resolvable in the
    `__main__` module at load time. `load_checkpoint` injects them.
